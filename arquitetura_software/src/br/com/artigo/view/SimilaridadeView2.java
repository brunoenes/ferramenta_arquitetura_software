...
public class SimilaridadeView2 extends JFrame {

	/**
	 * 
	 */
	private static final long serialVersionUID = -1582918380770778847L;
	...
	public static void main(String[] args) {
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					SimilaridadeView2 window = new SimilaridadeView2(null,null);
	...
	public SimilaridadeView2(ArrayList<InfoPacote> pacotes, ArrayList<InfoPacote> alterados) {
	...
	private void initialize() {
		...
		setResizable(false);
		setTitle("Similaridade - Melhorias");

		JScrollPane scroll = new JScrollPane();
		getContentPane().add(scroll, BorderLayout.CENTER);
		
		
		JButton btnVerificarNovaSimilaridade = new JButton("Gerar Classes");
		btnVerificarNovaSimilaridade.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				SimilaridadeView3 window = new SimilaridadeView3(pacotes,alterados);
				window.setVisible(true);
			}
		});
				...
				for (InfoClasse infoClasse : infoPacote.getClassesPacote()) {
					...
					if (!infoPacote.getClassePadrao().equals(infoClasse)) {
						...
						if(infoClasse.getPacoteAlterado() != null) {
							cor = infoClasse.getCorNova();
							simi = infoClasse.getSimNovoPacote();
						} else {
							cor = infoClasse.getCorAtual();
							simi = infoClasse.getSimilaridadeClasse();
						}
						
						if (i % 2 == 0) {
							Circulo circulo_1 = new Circulo(Color.decode(cor), infoClasse.getNomeClasse(),simi);
